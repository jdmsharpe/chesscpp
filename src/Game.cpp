...
bool Game::parseMove(const std::pair<std::string, std::string> input, std::pair<Position, Position> &output) const
{
    ...
    if (input.first.length() != 2 || input.second.length() != 2 ||
        !std::regex_search(input.first, legalMove) || !std::regex_search(input.second, legalMove))
    {
    ...
    output.first = {letterToIndex.at(input.first[0]), numberToIndex.at(input.first[1])};
    output.second = {letterToIndex.at(input.second[0]), numberToIndex.at(input.second[1])};
